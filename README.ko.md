...
캔버스 상단 **Add Node** 버튼으로 활성 workflow에 CLI agent 또는 Input 노드를 추가합니다. 노드가 선택된 상태에서는 버튼이 **Add Next**로 바뀌고, 새 노드를 선택 노드 뒤에 자동으로 연결합니다. 새 노드는 현재 캔버스 viewport 안에 배치됩니다.
...
Local LLM, Cloud API, Markdown Output 노드는 기존 workflow와 예제를 위해 내부적으로는 남아 있습니다. 하지만 완전히 구현되기 전까지 Core Nodes 추가 메뉴에는 Input만 노출합니다. Ollama는 placeholder가 아니라 실제 로컬 HTTP 호출을 수행하게 된 뒤 노출할 예정입니다.
